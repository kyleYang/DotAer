...
//	zipInfo.dosDate = (unsigned long) current;
	NSError *error;
	NSDictionary* attr = [[NSFileManager defaultManager] attributesOfItemAtPath:file error:&error]; //fileAttributesAtPath:file traverseLink:YES];
	if( attr )
	{
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	//	NSString * strPath = [NSString  stringWithCString:filename];
        NSString* strPath=[[[NSString alloc]initWithCString:filename encoding:NSUTF8StringEncoding] autorelease];
        NSLog(@"strPath:%@",strPath);//[strPath stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]);
		BOOL isDirectory = NO;
