...
@synthesize curUnzipFileId = _curUnzipFileId;
@synthesize totalUnzipFile = _totalUnzipFile;
@synthesize lastUIUpdateTS;
...
@interface Downloader() <DownloadTaskCallback,DownloadProgressCallback>
...
#pragma mark
#pragma mark DownloadProgressCallback
-(void)downloadTask: (DownloadTask*)tsk DidFinishWithError: (NSError*)error {
...
-(void)downloadTask: (DownloadTask*)tsk DownloadBytes: (NSInteger)dBytes OfTotal: (NSInteger)dAllBytes {
	BqsDownloaderItem *bid = tsk.attached;
    
    BqsDownloaderItem *di = [_netTasks objectForKey:[NSNumber numberWithInt:tsk.taskId]];
    assert(di == tsk.attached);

	
	//NSBqsLog(@"download progress: %d / %d", dBytes, dAllBytes);
	
	if(di != bid) {
		BqsLog(@"not contain the task! cancel it! url=%@", tsk.url);
		[tsk cancel];
		return;
	}
	
    bid.totalBytes = dAllBytes;
	bid.curBytes = dBytes;
    
    if (dAllBytes == 0) return;
    
    CGFloat percent = dBytes*100/dAllBytes;
    
    if([_delegate respondsToSelector:@selector(DownloadProgres:)]) {
		[_delegate DownloadProgres:percent];
	}
