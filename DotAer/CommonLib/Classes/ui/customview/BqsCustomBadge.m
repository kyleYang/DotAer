...
#define kPaddingVer 2
#define kPaddingHori 5

@interface BqsCustomBadge()
...
// I recommend to use the allocator customBadgeWithString
- (id) initWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withFont:(UIFont*)font withShining:(BOOL)shining 
{
	self = [super initWithFrame:CGRectMake(0, 0, 25, 25)];
	if(self!=nil) {
        
        if(nil != font) self.textFont = font;
        else self.textFont = [UIFont boldSystemFontOfSize:12];
		self.backgroundColor = [UIColor clearColor];
		self.badgeText = badgeString;
...
//        if(self.scaleFactor < 1.0) self.scaleFactor = 1.0;
		[self autoBadgeSizeWithString:badgeString];
	}
	return self;
}
...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
    ...
    if(nil != badgeString && [badgeString length] > 0) stringSize = [badgeString sizeWithFont:self.textFont];
//	CGFloat flexSpace;
//	if ([badgeString length]>=2) {
//    flexSpace = [badgeString length];
    rectWidth = stringSize.width + 2 * kPaddingHori; 
    rectHeight = stringSize.height + 2 * kPaddingVer;
    if(rectHeight > rectWidth) rectWidth = rectHeight;
...
+ (BqsCustomBadge*) customBadgeWithString:(NSString *)badgeString
{
//	return [[[self alloc] initWithString:badgeString withScale:1.0 withShining:YES] autorelease];
    return [self customBadgeWithString:badgeString withStringColor:[UIColor whiteColor] withInsetColor:[UIColor redColor] withBadgeFrame:YES withBadgeFrameColor:[UIColor whiteColor] withFont:[UIFont boldSystemFontOfSize:12] withShining:YES];
}


// Creates a Badge with a given Text, Text Color, Inset Color, Frame (YES/NO) and Frame Color 
+ (BqsCustomBadge*) customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withFont:(UIFont*)fnt withShining:(BOOL)shining
{
	return [[[self alloc] initWithString:badgeString withStringColor:stringColor withInsetColor:insetColor withBadgeFrame:badgeFrameYesNo withBadgeFrameColor:frameColor withFont:fnt withShining:shining] autorelease];
}
...
//		lineSize += self.scaleFactor*0.25;
//	}
...
- (void)drawRect:(CGRect)rect {
	...
	if ([self.badgeText length]>0) {
		[badgeTextColor set];
//		CGFloat sizeOfFont = 13.5*self.scaleFactor;
//		if ([self.badgeText length]<2) {
//			sizeOfFont += sizeOfFont*0.20;
//		}
//		UIFont *textFont = [UIFont boldSystemFontOfSize:sizeOfFont];
		CGSize textSize = CGSizeZero;
        if(nil != self.badgeText && [self.badgeText length] > 0) textSize = [self.badgeText sizeWithFont:textFont];
		[self.badgeText drawAtPoint:CGPointMake((rect.size.width/2-textSize.width/2), (rect.size.height/2-textSize.height/2)) withFont:self.textFont];
	}
