...
@interface HumDotaNavigation_Button : NSObject  //buttom navigation button
{
    BOOL _select;
}
...
@optional
-(void)HumDotaFrameCatSelectView_PopButton:(HumDotaNavigation_Button*)obj DidSelectCat:(NSString*)catId;

@end

@implementation HumDotaNavigation_Button
...
@implementation HumDotaButtomNav
    ...
    */
    ...
    for(NSString *sCatId in arrCatIds) {
        if([kDotaCat_News isEqualToString:sCatId]) {
            [self.arrAllButtons addObject:[[[HumDotaNavigation_Button alloc] initWithCatId:sCatId Name:NSLocalizedString(@"dota.buttom.nav.news",nil) NorImg:@"dota_frame_cat_news_nor.png" HotImg:@"dota_frame_cat_news_hilight.png" Callback:self]autorelease]];
        } else if([kDotaCat_Video isEqualToString:sCatId]) {
            [self.arrAllButtons addObject:[[[HumDotaNavigation_Button alloc] initWithCatId:sCatId Name:NSLocalizedString(@"dota.buttom.nav.video",nil) NorImg:@"dota_frame_cat_video_nor.png" HotImg:@"dota_frame_cat_video_hilight.png" Callback:self]autorelease]];
        } else if([kDotaCat_Photo isEqualToString:sCatId]) {
            [self.arrAllButtons addObject:[[[HumDotaNavigation_Button alloc] initWithCatId:sCatId Name:NSLocalizedString(@"dota.buttom.nav.photo",nil) NorImg:@"dota_frame_cat_photo_nor.png" HotImg:@"dota_frame_cat_photo_hilight.png" Callback:self]autorelease]];
        } else if([kDotaCat_Strategy isEqualToString:sCatId]) {
            [self.arrAllButtons addObject:[[[HumDotaNavigation_Button alloc] initWithCatId:sCatId Name:NSLocalizedString(@"dota.buttom.nav.strategy",nil) NorImg:@"dota_frame_cat_strategy_nor.png" HotImg:@"dota_frame_cat_strategy_hilight.png" Callback:self]autorelease]];
        } else if([kDotaCat_Simulator isEqualToString:sCatId]) {
            [self.arrAllButtons addObject:[[[HumDotaNavigation_Button alloc] initWithCatId:sCatId Name:NSLocalizedString(@"dota.buttom.nav.simulator",nil) NorImg:@"dota_frame_cat_simulator_nor.png" HotImg:@"dota_frame_cat_simulator_hilight.png" Callback:self]autorelease]];
        }
    }
    
    
    for (int i= 0; i<self.arrAllButtons.count;i++ ) {
        HumDotaNavigation_Button *obj = [self.arrAllButtons objectAtIndex:i];
        CGRect frame = obj.btn.frame;
        obj.btn.frame = CGRectMake(i*64, 0, CGRectGetWidth(frame), CGRectGetHeight(frame));
...
-(void)HumDotaFrameCatSelectView_PopButton:(HumDotaNavigation_Button*)obj DidSelectCat:(NSString*)catId {
    ...
    for (HumDotaNavigation_Button *bg in self.arrAllButtons) {
        bg.select = FALSE;
    }
    obj.select = TRUE;
