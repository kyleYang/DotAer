...
- (void)refreshLastUpdatedDate {
	...
	if (_delegate&&[_delegate respondsToSelector:@selector(egoRefreshTableHeaderDataSourceLastUpdated:)]) {
		...
		[formatter setAMSymbol:NSLocalizedString(@"pg.time.forenoon", nil)];
		[formatter setPMSymbol:NSLocalizedString(@"pg.time.afternoon", nil)];
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {	
	...
	if (_state == EGOOPullRefreshLoading) {
		...
		if (_state == EGOOPullRefreshPulling && scrollView.contentOffset.y > -self.pullDistance&&scrollView.contentOffset.y < 0.0f &&!_loading) {
			[self setState:EGOOPullRefreshNormal];
		} else if (_state == EGOOPullRefreshNormal && scrollView.contentOffset.y < -self.pullDistance && !_loading) {
        ...
        [self refreshLastUpdatedDate];
        [self performSelector:@selector(egoRefreshScrollViewDidEndDragging:) withObject:scrollView afterDelay:0.5];
    }
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y <= -self.pullDistance && !_loading) {
