...
CGFloat distancePad(CGPoint a, CGPoint b);


@implementation LeavesPadView

@synthesize delegate;
@synthesize leafEdge, currentPageIndex, backgroundRendering, preferredTargetWidth;
@synthesize leftAttributedText = _leftAttributedText,rightAttributedText = _rightAttributedText;
...
- (void) setUpLayers {
    ...
    topPageShadow.contentsScale = scale;
...
- (void) initialize {
	...
	pageCache = [[LeavesPadCache alloc] initWithPageSize:self.bounds.size];
    leftCustomLinks = [[NSMutableArray alloc] init];
    rightCustomLinks = [[NSMutableArray alloc] init];
...
- (void)dealloc {
    ...
    [_leftAttributedText release];
    [_rightAttributedText release];
	[pageCache release];
    [leftCustomLinks release];
    [rightCustomLinks release];
...
- (void)reloadCurrentPage:(NSUInteger)page    //editby kyle yang for asynchronous image download
{
	...
	if (currentPageIndex < numberOfPages) {
        ...
        _leftTextFrame = [pageCache.dataSource getTextFrameAtPage:currentPageIndex isLeft:YES];
        _rightTextFrame = [pageCache.dataSource getTextFrameAtPage:currentPageIndex isLeft:NO];
        self.leftAttributedText = [pageCache.dataSource getAttributedTextAtPage:currentPageIndex isLeft:YES];
        self.rightAttributedText = [pageCache.dataSource getAttributedTextAtPage:currentPageIndex isLeft:NO];
        [pageCache.dataSource leafAddLinker:self withLines:nil withFrame:_leftTextFrame isLeft:YES];
        [pageCache.dataSource leafAddLinker:self withLines:nil withFrame:_rightTextFrame isLeft:NO];
        ...
        _leftTextFrame = nil;
        _rightTextFrame = nil;
        self.rightAttributedText = nil;
        self.leftAttributedText = nil;
...
- (void) setDataSource:(id<LeavesPadViewDataSource>)value {
...
- (void)setLeftAttributedText:(NSAttributedString *)aleftAttributedText
{
    [_leftAttributedText release];
    _leftAttributedText = [aleftAttributedText retain];
    [self removeAllLeftCustomLinks];
}

- (void)setRightAttributedText:(NSAttributedString *)arightAttributedText
{
    [_rightAttributedText release];
    _rightAttributedText = [arightAttributedText retain];
    [self removeAllRightCustomLinks];
}


- (CTFrameRef)getCurentFrame{
    return _leftTextFrame;
}
...
-(void)addCustomLink:(NSURL*)linkUrl inRange:(NSRange)range isLeft:(BOOL)left{
    ...
    if(left)
        [leftCustomLinks addObject:link];
    else
        [rightCustomLinks addObject:link];
    
}

-(void)removeAllLeftCustomLinks {
	[leftCustomLinks removeAllObjects];
	
}

-(void)removeAllRightCustomLinks{
    [rightCustomLinks removeAllObjects];
}
...
-(NSTextCheckingResult*)linkAtCharacterIndex:(CFIndex)idx isLeft:(BOOL)left{
...
//	}
    if(left){
        [leftCustomLinks enumerateObjectsUsingBlock:^(id obj, NSUInteger aidx, BOOL *stop)
         {
        ...
        [rightCustomLinks enumerateObjectsUsingBlock:^(id obj, NSUInteger aidx, BOOL *stop)
         {
             NSRange r = [(NSTextCheckingResult*)obj range];
             if (NSLocationInRange(idx, r)) {
                 foundResult = [[obj retain] autorelease];
...
-(NSTextCheckingResult*)linkAtPoint:(CGPoint)point {
    ...
    CFArrayRef lines;
    BOOL isLeft = NO;
    if(point.x<pageSize.width/2)
        isLeft = YES;
    else
        point.x = point.x - pageSize.width/2;
    
    if(isLeft)
        lines = CTFrameGetLines(_leftTextFrame);
    else
        lines = CTFrameGetLines(_rightTextFrame);
    ...
    if (isLeft)
        CTFrameGetLineOrigins(_leftTextFrame, CFRangeMake(0,0), origins);
    else
        CTFrameGetLineOrigins(_rightTextFrame, CFRangeMake(0,0), origins);
	...
	for (int lineIndex=0 ; lineIndex<nbLines ; ++lineIndex) {
		...
		if (CGRectContainsPoint(lineRect, point)) {
			...
			link = ([self linkAtCharacterIndex:idx isLeft:isLeft]);
			if (link) return link;
		}
	}
	return nil;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	UITouch *touch = [event.allTouches anyObject];
	CGPoint touchPoint = [touch locationInView:self];
	BOOL dragged = distancePad(touchPoint, touchBeganPoint) > [self dragThreshold];
