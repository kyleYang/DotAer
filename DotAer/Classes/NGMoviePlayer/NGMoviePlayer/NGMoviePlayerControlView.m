...
@interface NGMoviePlayerControlView ()<NGVideoTypeDelegate> {
...
@property (nonatomic, strong, readwrite) UIButton *downloadControl;
...
//        _settingControl.contentMode = UIViewContentModeCenter;
        ...
        [_settingControl setBackgroundImage:[UIImage imageNamed:@"NGMoviePlayer.bundle/videoTypedown"] forState:UIControlEventTouchDown];
...
//        _downloadControl.showsTouchWhenHighlighted = YES;
        _downloadControl.frame = (CGRect) { .size = CGSizeMake(70.f, 60.f) };
//        _downloadControl.contentMode = UIViewContentModeCenter;
        [_downloadControl setBackgroundImage:[UIImage imageNamed:@"NGMoviePlayer.bundle/download"] forState:UIControlStateNormal];
        [_downloadControl addTarget:self action:@selector(handleDownloadButtonPress:) forControlEvents:UIControlEventTouchUpInside];
        [_settingControlsView addSubview:_downloadControl];
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    return  insideTopControlsView || insideBottomControlsView|insideSettinControlsView;
}
...
- (void)NGVideoType:(NGVideoType *)type didSelectAtIndex:(NSUInteger)index{
    ...
    [HumDotaUserCenterOps intVaule:index saveForKey:kScreenPlayType];
    NSString *vidoStep = @"";
    switch (index) {
