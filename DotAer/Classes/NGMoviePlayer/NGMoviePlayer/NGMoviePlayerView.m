...
#define kMinVideoChangeLength 10
#define kMaxTimeInterVal (5*60) // 5 minutes
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if (self.controlsVisible || self.placeholderView.alpha > 0.f) {
        ...
        if (self.controlsView.volumeControl.expanded||self.controlsView.stepExpaned) {
            return NO;
        }
...
- (void)setup {
    ...
    _controlsView.videoTitle.text = self.videoName;
    ...
    placeholderView.infoText = self.videoName;
    _placeholderView = placeholderView;
    placeholderView.backBtn.hidden = YES;
    [self addSubview:_placeholderView];
    ...
    self.speedView = [[NGSpeedUpView alloc] initWithFrame:CGRectMake(0, 0, 200, 80)];
