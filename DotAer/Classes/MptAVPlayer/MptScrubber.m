...
@interface MptSliderValuePopupView : UIView  
...
@implementation MptSliderValuePopupView
...
- (void)setTime:(NSTimeInterval)time {
    if (_time != time) {
        _time = time;
        self.text = MptAVPlayerGetTimeFormatted(time);
...
@interface MptScrubber () {
    CGPoint _beganTrackingLocation;
    float _realPositionValue;
    MptSliderValuePopupView *valuePopupView; 
}

@property (atomic, assign, readwrite) float scrubbingSpeed;
@property (atomic, assign) CGPoint beganTrackingLocation;
@property (nonatomic, retain) UIView *playableView;

@end


@implementation MptScrubber

@synthesize scrubbingSpeed;
@synthesize beganTrackingLocation;
@synthesize scrubbingSpeeds,scrubbingSpeedChangePositions;
@synthesize playableValue = _playableValue;
@synthesize playableValueColor = _playableValueColor;
@synthesize fillColor;
@synthesize playableValueRoundedRectRadius = _playableValueRoundedRectRadius;
@synthesize showPopupDuringScrubbing = _showPopupDuringScrubbing;
...
- (void)dealloc{
    ...
    _playableValueColor = nil;
    self.fillColor = nil;
    [_playableView release]; _playableView = nil;
    [super dealloc];
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _playableValueColor = [UIColor colorWithRed:0.0f green:1.0f blue:0.0f alpha:0.7f];
...
- (void)constructSlider {
    valuePopupView = [[MptSliderValuePopupView alloc] initWithFrame:CGRectZero];
...
- (void)positionAndUpdatePopupView {
    CGRect thumbRect = [self thumbRectForBounds:self.bounds 
                                      trackRect:[self trackRectForBounds:self.bounds]
                                          value:self.value];
    CGFloat height = 35.f;
    CGRect popupRect = CGRectOffset(thumbRect, 0.f, -25);
