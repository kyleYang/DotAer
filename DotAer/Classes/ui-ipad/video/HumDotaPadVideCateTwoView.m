...
@interface HumDotaPadVideCateTwoView()<HumVideoPadTableCellDelegate,MptAVPlayerViewController_Callback>
...
@implementation HumDotaPadVideCateTwoView
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HumVideoPadTableCell *cell = (HumVideoPadTableCell *)[aTableView dequeueReusableCellWithIdentifier:cellIden];
    if (!cell) {
        cell = [[[HumVideoPadTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIden] autorelease];
    }
    cell.delegate = self;
...
-(float)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGSize size = [info.title sizeWithFont:kTitleFont constrainedToSize:CGSizeMake(CGRectGetWidth(self.bounds)-2*kOrgX-kCoverWidth-kVTGap, 1000) lineBreakMode:UILineBreakModeWordWrap];
    height += (size.height > kCoverHeigh)? size.height:kCoverHeigh;
    
    size = [info.summary sizeWithFont:kSumaryFont constrainedToSize:CGSizeMake(CGRectGetWidth(self.bounds)-2*kOrgX, 1000) lineBreakMode:UILineBreakModeWordWrap];
